...
def migrate(env, version):
    ...
    _logger.info("FACTURES = %s" % invoices_data)
    for invoice in invoices_data:
        ...
        wallet = env["res.partner.backend"].search([("partner_id", "=", partner_id.id), ("type", "=", "comchain")], limit=1)
        if not wallet:
            continue
        values.update({
    ...
    env.cr.execute('ALTER TABLE account_invoice DROP COLUMN temporary_comchain_amount_credited')
    env.cr.execute('ALTER TABLE account_invoice DROP COLUMN temporary_comchain_amount_to_credit')
