...
class ResPartnerBackend(models.Model):
    ...
    def credit_wallet(self, amount=0):
        ...
        try:
            odoo_wallet.unlock(self.alt_currency_id.comchain_odoo_wallet_password)
        except Exception as e:
            _logger.error(tools.format_last_exception())
            return {
                "success": False,
        ...
        try:
            response = odoo_wallet.transferOnBehalfOf(
                f"0x{self.alt_currency_id.safe_wallet_partner_id.lcc_backend_ids[0].comchain_id}",
    ...
    def get_wallet_data(self):
        ...
        if self.type == "comchain":
            data = [
                f"comchain:{self.alt_currency_id.name}",
                self.comchain_id,
            ]
        return data
