...
def migrate(env, version):
    ...
    for invoice in invoices_data:
        ...
        wallet = env["res.partner.backend"].search([("partner_id", "=", partner_id.id), ("type", "=", "cyclos")], limit=1)
        if not wallet:
            continue
        values = {
            "amount" : invoice[2],
    ...
    env.cr.execute('ALTER TABLE account_invoice DROP COLUMN temporary_cyclos_amount_credited')
