...
class CreditRequest(models.Model):
    ...
    partner_id = fields.Many2one('res.partner', related='wallet_id.partner_id', readonly=True)
    ...
    invoice_id = fields.Many2one('account.invoice', string='Linked Invoice')
    ...
    def unlink(self):
        for request in self:
            if request.state == "pending" or (request.invoice_id and request.invoice_id.state != "draft"):
                raise UserError("You can't delete a credit request linked with a confirmed or paid invoice.")
            elif request.state == "error":
                raise UserError("You can't delete a credit request in Error. Please solve the issue.")
            elif request.state == "done":
                raise UserError("You can't delete a done credit request. Please archive it instead.")

            if request.order_id:
                if (request.order_id and request.order_id.state not in ("draft", "cancel")):
    ...
    def credit_wallet(self):
        ...
        for record in self:
            ...
            if record.amount == 0 or not record.wallet_id:
                raise Exception("Missing information in the credit request - Top up cancelled.")
    ...
    def get_credit_request_data(self):
        ...
        return {
                    ...
                    "order_url": self.order_id.get_portal_url() if self.order_id else "",
                    "amount": self.amount,
