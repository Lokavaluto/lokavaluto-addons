...
class TestPartnerService(TransactionComponentCase):
    def setUp(self):
        ...
        self.sender = self.ResPartner.create({"name": "Sender", "user_id": self.sender_user.id})
        ...
        self.PartnerCheckTransactionGetParams = self.env.datamodels["partner.check.transaction.get.params"]
        self.sender_to_allowed_recipient_get_params = self.PartnerCheckTransactionGetParams(
            sender_wallet_ident=self.sender_wallet.name,
            recipient_wallet_ident=self.recipient_allowed_wallet.name
        )

        self.sender_to_not_allowed_recipient_get_params = self.PartnerCheckTransactionGetParams(
            sender_wallet_ident=self.sender_wallet.name,
    ...
    def test_is_transaction_allowed(self):
        ...
        collection = self.env["lokavaluto.private.services"].with_user(self.sender_user).browse(1)
    ...
    def test_rule_allowing_all_senders(self):
        ...
        another_sender_user = self.ResUsers.create({"name": "Another Sender", "login": "bar"})
        another_sender = self.ResPartner.create({"name": "Another Sender", "user_id": another_sender_user.id})
        ...
        another_sender_to_allowed_recipient_get_params = self.PartnerCheckTransactionGetParams(
            sender_wallet_ident=another_sender_wallet.name,
            recipient_wallet_ident=self.recipient_allowed_wallet.name
        )
        another_sender_to_not_allowed_recipient_get_params = self.PartnerCheckTransactionGetParams(
            sender_wallet_ident=another_sender_wallet.name,
            recipient_wallet_ident=self.recipient_not_allowed_wallet.name
        )

        collection = self.env["lokavaluto.private.services"].with_user(another_sender_user).browse(1)
        with collection.work_on("res.partner.backend") as work:
            service = work.component(usage="partner")
            result = service.is_transaction_allowed(
