...
from odoo.addons.partner_profiles_portal.controllers.portal_structure_profile import (
    CustomerPortalStructureProfile,
)
...
class CustomerPortalStructureProfileLcc(CustomerPortalStructureProfile):
    def _get_main_profile_fields(self):
        fields = super(
            CustomerPortalStructureProfileLcc, self
        )._get_main_profile_fields()
        lcc_fields = [
            "main_industry_id",
    ...
    def _get_main_boolean_structure_fields(self):
        fields = super(
            CustomerPortalStructureProfileLcc, self
        )._get_main_boolean_structure_fields()
        lcc_fields = [
            "main_itinerant",
            "main_accept_coupons",
            "main_accept_digital_currency",
    ...
    def _get_page_saving_main_values(self, kw):
        values = super(
            CustomerPortalStructureProfileLcc, self
        )._get_page_saving_main_values(kw)
        if kw.get("main_website_description", "") != "":
            values.update({"website_description": kw["main_website_description"]})
        if kw.get("main_discount", "") != "":
            values.update({"discount": kw["main_discount"]})
        return values
