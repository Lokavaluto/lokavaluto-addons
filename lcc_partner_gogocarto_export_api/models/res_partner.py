    ...
    def add_fields(self, element, export_fields):
        for field in export_fields:
            if field.name == "team_id":
                ...
                self.__add_computed_node(
                    element, "itinerant", self._get_itinerant_label
                )
            elif field.name == "currency_exchange_office":
                self.__add_computed_node(
                    element, "exchange_counter", self._get_exchange_counter_label
                )
            elif field.name == "team_id":
                self.__add_computed_node(
                    element, "local_group", self._get_team_id_label
                )
            elif field.name == "industry_id":
                self.__add_computed_node(
                    element, "industry_id", self._get_industry_id_label
                )
            else:
